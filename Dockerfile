...
RUN curl https://dl.google.com/android/android-sdk_r24.4.1-linux.tgz /opt/android-sdk.tgz \
  | tar xz -C ${HOME} \
  && ( while [ 1 ]; do sleep 5; echo y; done ) | ${ANDROID_HOME}/tools/android update sdk --no-ui \
  && ( while [ 1 ]; do sleep 5; echo y; done ) | ${ANDROID_HOME}/tools/android update sdk --no-ui --filter android-23  \
  && ( while [ 1 ]; do sleep 5; echo y; done ) | ${ANDROID_HOME}/tools/android update sdk --no-ui --filter build-tools-23.0.2
...
# Install the NDK
RUN curl https://dl.google.com/android/repository/android-ndk-r12b-linux-x86_64.zip > android-ndk-r12b-linux-x86_64.zip \
...
RUN mkdir ${ANDROID_STANDALONE_TOOLCHAINS_PATH} \
  && for i in arm arm64 x86 x86_64; do \
    $ANDROID_NDK_HOME/build/tools/make_standalone_toolchain.py  \
      --arch $i --api 21 --install-dir ${ANDROID_STANDALONE_TOOLCHAINS_PATH}/$i;\
  done
